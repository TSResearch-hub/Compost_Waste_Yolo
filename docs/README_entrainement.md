# Entraînement (YOLO / RT-DETR) — détection d'intrus dans le compost

Partie entraînement et évaluation du dépôt : le paquet `compost_detection`
(`src/`), ses scripts (`scripts/`), ses configs et ses modèles. La
classification est **orientée matériau** : on annote la matière (`Plastique`,
`Metal`...), pas l'objet.

**Toutes les commandes de ce guide se lancent depuis la racine du dépôt.**

Les classes sont définies à UN SEUL endroit : [`configs/data.yaml`](../configs/data.yaml).

## Installation

Python 3.10+ requis. **Un seul environnement virtuel pour tout le dépôt**, à la
racine (le même sert aux interfaces) :

```bash
python3 -m venv venv
source venv/bin/activate           # Windows : venv\Scripts\activate
pip install -r requirements.txt    # installe aussi le paquet compost_detection
```

Sur une machine d'entraînement sans interfaces, `pip install -e ".[app,dev]"`
suffit (le `pyproject.toml` est à la racine).
...
(ex. `models/v0_pretrain_yolov8n.pt`) — c'est le point de départ **canonique**
de chaque réentraînement (jamais le fine-tuné précédent : le split peut changer
quand le dataset grandit, seul un départ du pré-entraîné garantit que le test
n'a jamais été appris). Versions et provenance des modèles :
...
`models/v0_pretrain_rtdetr-l.pt`, puis `python scripts/retrain.py --pretrain
models/v0_pretrain_rtdetr-l.pt --batch 4`. Les deux modèles sont alors évalués
sur le **même** test compost — comparaison directe.
...
## Échanges avec l'interface d'annotation

**Entrée — récupérer les données annotées.** L'interface d'annotation écrit
dans `dataset_recolte/` (racine du dépôt) ; `update_dataset.py` l'intègre
directement (c'est sa source par défaut). Pour la voie manuelle via
`prepare_dataset.py` :
...
**Sortie — déployer le modèle vers les interfaces.** Le `best.pt` retenu
(après évaluation et inscription dans [`models/README.md`](../models/README.md))
devient le modèle déployé :

```bash
cp models/vN_....pt weights/best.pt
```

## Entraînement sur Google Colab (GPU)

Le code se développe et se versionne en local (repo `Compost_Waste_Yolo`) ;
les entraînements réels tournent sur Colab.
...
models/             # modèles versionnés v0/v1/v2 — voir models/README.md (les .pt sont gitignorés)

...
def weights_kind(weights):
    ...
    models/v0_pretrain_yolov8n.pt             -> 'pretrain'
    ailleurs / non reconnu                    -> 'model'
    """
    p = Path(str(weights)).resolve()
    name = p.parent.parent.name if p.parent.name == "weights" else p.stem
    # la convention models/README.md préfixe les fichiers par leur version
    # (v0_pretrain_..., v1_rtdetr...) : on saute ce préfixe avant de lire le rôle
    parts = name.lower().split("_")
    if parts and re.fullmatch(r"v\d+", parts[0]):
        parts = parts[1:] or ["model"]
    kind = parts[0].split("-")[0]
